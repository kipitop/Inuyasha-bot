...
export default async function (m, conn = { user: {} }) {
    ...
    console.log(`
╭───────────────────────────────⭓
│ 🧑 BOT     : ${chalk.black(chalk.bgCyanBright(`${me} ~ ${conn.user.name}`))}
│ 📅 HORA    : ${chalk.cyanBright(oraColombia)}
│ 🧾 EVENTO  : ${chalk.cyanBright(m.messageStubType || 'WAMessageStubType')}
│ 💾 TAMAÑO  : ${chalk.cyanBright(`${filesize} [${filesize === 0 ? 0 : (filesize / 1000 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1)} ${['', 'K', 'M', 'G', 'T', 'P'][Math.floor(Math.log(filesize) / Math.log(1000))] || ''}B]`)}
│ 👤 REMIT.  : ${chalk.white(sender)}
│ 📊 EXP/LVL : ${chalk.cyanBright(`${m.exp || '?'} |${user?.exp || 0}|${user?.limit || 0}|LVL:${user?.level || 0}`)}
│ 💬 TIPO    : ${chalk.yellow(m.mtype?.replace(/message$/i, '').replace('audio', m.msg?.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) || '')}
│ 🗂️ CHAT    : ${chalk.cyanBright(chatName)}
│ 🆔 MSG ID  : ${chalk.gray(m.key?.id || 'N/A')}
╰───────────────────────────────⭓
`);
